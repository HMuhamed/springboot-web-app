...
public class ControlsPanel extends JPanel implements ActionListener {
	...
	private String[] algorithmsName = { 
			"Breadth-First Search", "Depth-First Search", "Best-First Search", "A*", "Bellman-Ford", "Dijkstra"
		};
	...
	SearchAlgorithms algo = null;
	    ...
	    public ControlsPanel(int width, int height, GridPanel gridPanel) {
	        ...
	        this.addFocusListener(new FocusListener() {
	            ...
	            public void focusGained(FocusEvent e) {
	                // Optional: handle focus gained if necessary
	            }
	        });
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < grid.getRows(); i++) {
					for (int j = 0; j < grid.getCols(); j++) {
						...
						if (!node.isStart() && !node.isFinish() && rand.nextFloat() > 0.9) {
							node.setWall(true);
						}
					}
				}
		...
		algorithms.setEnabled(true); // Disables the JComboBox
		algorithms.setEditable(true); // Makes the JComboBox non-editable
		algorithms.setFocusable(false); // Prevents it from gaining focus
		...
		algorithms.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				MyUtils.algorithm = algorithms.getSelectedIndex()+2;
			}
		});
		...
		size = new JSlider(3, 60, Node.size);
		...
		size.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				if (MyUtils.solving) {
			       ...
			       // Inverto vlerën e slider-it
		        int maxValue = size.getMaximum(); // 60
		        int minValue = size.getMinimum(); // 3
		        int sliderValue = size.getValue();
		        
		        // Inverto vlerën e slider-it: p.sh., kur është në 3 të shfaqë 60 dhe anasjelltas
		        int invertedValue = maxValue - (sliderValue - minValue);

		        int cellSize = invertedValue / 5 * 5;
